...
knobi_retro<-function(knobi_results,yR=NULL,yR0=NULL,nR=NULL,plot_out=F,plot_filename=NULL,plot_dir=NULL){
  ...
  retro_plot<-ggplot2::ggplot() + ggplot2::theme_bw() +
    ggplot2::geom_point(data=df[c(1,nrow(df)),],ggplot2::aes(x=x,y=df[c(1,nrow(df)),]$y,color=bleg),size=3, show.legend=FALSE) +
    ggplot2::geom_text(data=df[c(1,nrow(df)),],ggplot2::aes(x=x,y=df[c(1,nrow(df)),]$y,color=bleg,
                                                            label=Year,vjust=-1),size=4, show.legend = FALSE) +
    ggplot2::geom_point(data=df,ggplot2::aes(x=x,y=df$y,color=bleg),show.legend=FALSE) +
    ggplot2::geom_path(data=df,ggplot2::aes(x=x,y=df$y,color=bleg)) +
    ggplot2::labs(title=btit, subtitle=knobi_results$data$Stock,
                  x =baxis, y = "Surplus Production (SP)") +
    ggplot2::geom_line(data=df_plot,ggplot2::aes(x=df_plot$B,y=df_plot$SP,color=factor)) + ggplot2::ylim(min_y,max_y) +
