...
if (is.null(environmental$lag)){
  ...
  for(j in env_names){
    data_env[[j]]=data.frame(knobi_results$data$SP,knobi_results$data$Average_Biomass,
                             knobi_results$data$years)
    ...
    for (i in 1:nlag_j){
      ...
      if(!is.na(knobi_results$data$Recruitment[1])){
        ...
        if(length(ind)>0){data_env[[j]][,4+i]=env0[[j]][ind:ind1]} else {
          warning('The length of the environmental variable is not enough to use the number of input lags')}
      }
    }
    ...
    if(knobi_results$control$method=="Biomass"){
      if(!is.na(knobi_results$data$Recruitment[1])){
        colnames(data_env[[j]])=c("SP","B","years","R",paste0(j,"_lag0"),vec_env)
      } else {colnames(data_env[[j]])=c("SP","B","years",paste0(j,"_lag0"),vec_env)}
    } else {
      if(!is.na(knobi_results$data$Recruitment[1])){
        colnames(data_env[[j]])=c("SP","SSB","years","R",paste0(j,"_lag0"),vec_env)
      } else {colnames(data_env[[j]])=c("SP","SSB","years",paste0(j,"_lag0"),vec_env)}
    }

    if(knobi_results$control$method=="Biomass"){
      if(!is.na(knobi_results$data$Recruitment[1])){names<-c("SP","B","R")
      } else {names<-c("SP","B")}
    } else {
      if(!is.na(knobi_results$data$Recruitment[1])){
        names<-c("SP","SSB","R")
    ...
    env[[j]]<-data_env[[j]][,c(names,paste0(j,"_lag0"),vec_env)]
    ...
    env[[j]]=env[[j]][,c("SP",paste0(j,"_lag0"),vec_env)]

    cor=round(cor(as.matrix(env[[j]]),use="na.or.complete"),4)
    cor=cor[,1]; cor=abs(cor[-1])

    res_env$selected_lag[[j]]=(which(max(cor)==cor)-1)
    res_env$lag_cor[[j]]=max(cor)
  ...
  for(j in env_names){
    ...
    cor=round(cor(as.matrix(df_env),use="na.or.complete"),4)
    cor=cor[2,]
    res_env$lag_cor=abs(cor[-c(1:3)])
}
...
if(multicovar==F){
  ...
  if(is.null(environmental$selected_var)){
    select<-which.max(res_env$lag_cor)
  ...
  # x.max_a = z.max_a = x.max_m = z.max_m = NULL
  #
  # for(i in 1:grid.lines){
  #   maxim<-max(z.pred_m[,i])
  #   z.max_m<-c(z.max_m,maxim)
  #   index<-which.max(z.pred_m[,i])
  #   x.max_m[i]<-x.pred_m[index]
  #   maxim<-max(z.pred_a[,i])
  #   z.max_a<-c(z.max_a,maxim)
  #   index<-which.max(z.pred_a[,i])
  #   x.max_a[i]<-x.pred_a[index]
  # }
  #
  #
  # Bmsy_m<-as.numeric((-a_m*(1/b_m))^((1/(n_m-1)))*(n_m^(-1/(n_m-1))))
  # Bmsy_a<-as.numeric((-a_a*(1/b_a))^((1/(n_a-1)))*(n_a^(-1/(n_a-1))))
  #
  # MSY_m=(a_m*Bmsy_m+b_m*Bmsy_m^n_m) #*exp(1)^{c_m*y.pred}
  # MSY_a=(a_a*Bmsy_a+b_a*Bmsy_a^n_a) #+c_a*y.pred
  #
  #
  # Fmsy=as.numeric(a_m*(1-(1/n_m)))
  #
  #
  #
  # points3D(x.max_a, y.pred2, z.max_a, add=T,
  #          col = "skyblue", type = "h", pch = 20)
  #
  # points3D(x.max_m, y.pred2, z.max_m, add=T,
  #          col = "skyblue", type = "h", pch = 20)
  ...
  ref_pts_add = RF(model_Schaefer_Mult)
  res_env$ref_pts = list(ref_pts_mult=ref_pts_mult,ref_pts_add=ref_pts_add)
