...
* `env`: data frame containing the values of each of the environmental variable(s) in one column. Each row represents a year.
* `years`: time series of years corresponding to the environmental variable(s).


In the object `list` we provide the optional settings for the environmental fit. In this case, for example, we are going to set `nlag=5`. This argument is used to test all the lags smaller or equal to `nlag` (numeric) through cor.test function. The lag corresponding to the highest pearson correlation among the base KBPM surplus production residuals and the lag environmental covariable values is considered in the environmental model. This means that the *residuals<sub>t</sub>* is related to *X<sub>t-lag</sub>* being *X* the environmental variable. The length of this argument must be equal to the number of environmental variables included. By default, `nlag=3`.
...
* `selected_lag`: Estimated lag corresponding to the one reporting the highest correlation between the environmental variable(s) and the base KBPM surplus production residuals (derived if lag is not fixed) or the fixed lag(s) and its corresponding correlation.
...
* `error`: List with the error measures:
...
In this case, 3D plots are never reported, and the output list slightly differs from the previous one.

* Lag(s) from `selected_lag` are now the input value(s) of `lag` argument.
...
In this case, `env_aic` is also returned, a data frame with the AIC values obtained for the AR model considering each environmental variable(s) for each lag among the AIC value for the AR model considering only the KBPM base SP residuals and the correlation plot shows this values.
