...
For simplicity, the output slot `$fit$ref_pts` provides the RPs estimates for a value of the centered environmental variable equal to 0 (*X=0*), see the following example.
...
$$F_{msy}(X)=\frac{r}{p}\left(1-\frac{1}{p+1}\right) cX_t$$
$$MSY(X)=B_{msy}(X)*F_{msy}(X)$$
$$K(X)=K$$

where *r*, *p*, *K* and *c* are the model parameter estimates and *X* the centered environmental variable.
...
where *r*, *p*, *K* and *c* are the model parameter estimates.
