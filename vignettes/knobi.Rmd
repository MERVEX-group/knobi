...
In this vignette we will try to illustrate the use of the `knobi` package for its correct functioning in the simplest way possible. For that, at first, the formulation of the Known biomass production models (KBPMs) will be detailed. Later on, the use of the functions contained in the package will be illustrated with some examples of their performance.
...
For the correct understanding of the KBPMs models we start reviewing the surplus production models (SPMs) framework. Then, we focus in the KBPMs formulation to provide the required knowledge for a correct package use.
...
In contrast to the traditional SPMs, KBPMs use as input data a biomass time series produced by other stock assessment models instead of a biomass index and thereby avoid the imprecision associated with estimating the catchability coefficient. 
...
Schaefer model (1954) corresponds to *p=1* (then the production curve is symmetric and *SP<sub>max</sub>* is equal to *K/2*).

&nbsp;

## 2. `knobi` package ##

In this section the main features of `knobi` package are described. This package allows: 
1. The KBPM fit to the stock through the `knobi_fit` function (main function).
2. The retrospective analysis by the use of `knobi_retro`.
3. The estimation of the effects of environmental variability via `knobi_env`.
4. The estimation of future projections for the stock with `knobi_proj`.
...
The data is downloaded using `icesSAG` package and saved `hake_n` object. Then the data list for `knobi_fit` is created. Note that the time period goes from 1978 to 2020.
...
data$years <- hake_n$Year                # Years (optional)
```
...
There is the possibility of defining another `control` settings such as `start_r`, `start_K` or `start_p`, optional start values of the model parameters $r$ (intrincis growth rate), $K$ (maximum population size) and $p$ (shape parameter in Pella-Tomlinson model), respectively.

&nbsp;

After preparing both lists, `data` and `control`, we can apply the `knobi_fit` function over them for the adjustment of the KBPM model.

```{r,eval=FALSE}
hake_n_results <- knobi_fit(data, control,
                            plot_out=TRUE,             # This argument allows us to create, in addition 
                            ...
                            plot_filename="hake_n",    # Optional name of the folder if plot_out=T. 
                                                       # "knobi_results" by default (b.d.)
                            plot_dir="Z:/knobi/HAKE/") # Optional folder directory (current one b.d.) 
```
...
* `goodness_of_fit`: List with goodness-of-fit measures measures:
  ...
  + `error_table`: Data frame measures of estimates accuracy (error measures comparing observed an estimated values): Standard error of the regression (SER), coefficient of determination (R-squared), adjusted coefficient of determination (adj-R-squared), Akaike information criterion (AIC), root-mean-squared error (RMSE) and the mean absolute percentage error (MAPE). 
...
The estimated surplus production curves from the retrospective analysis are plotted. The plot is displayed in the plot window and saved (note that `plot_out=T`) in the provided directory and file, that are the same as in the previous function example.
...
knobi_retro(hake_n_results,yR=c(2005,2010,2015),plot_out=FALSE)
```
...
1.  The analysis of the correlation between the environmental variable(s) and the SP KBPM residuals through the pearson's correlation or through the study of autoregressive models; 
2.  The choice of the variable and its corresponding lag that will be fitted by the models; 
3.  The KBPM environmental fit.

&nbsp;

For the step (3), the KBPM environmental fit, environmental effects are included as additive and multiplicative effects in the KBPM base formulation (equation 3).
...
hake_n_environmental <- knobi_env(hake_n_results,       # previous results
                                 ...
                                 control,               # the environmental settings
                                 plot_out=TRUE,
                                 plot_filename="hake_n",
                                 plot_dir="Z:/knobi/HAKE/"))
```
...
Result plots are shown in the plot window and also saved in the same provided directory and file like in the previous function example. In this case, the first plot reports the correlation analysis between the environmental variable(s) and the base KBPM surplus production residuals. The last one reports the fitted values of the base model (no environmental information) and of the environmental ones.
...
There is the possibility of obtain 3D plots that reports the surplus production curve conditioned to a grid of environmental values, through the argument `control$plot3d=TRUE`.
...
In this case, a list named `plots3D` is added to the output list of `knobi_env` with the 3D plots objects.
...
There is the possibility of fixing lag value(s) in the relation among the surplus production and the environmental variable, using `lag` instead `nlag` inside `control`.
...
In this case, 3D plots are never reported, and a plot with the pearson correlation between the environmental variables is displayed. The output list slightly differs from the previous one.
...
`knobi_proj` mandatory input is the object returned by `knobi_fit` and the following information:

* `env_results`: The output object of `knobi_env` function. This argument is optional. It has to be provided if you also want to consider the environmental fit in the projections.
* `n_y`: The number of years for projections. If not provided, enter `end_y` (the next argument).
* `end_y`: The final year of the projections. If not provided, enter `n_y` (the argument above). If both are provided, the function uses `n_y`.
...
The plots results are saved and displayed in the plot window. If environmental information is provided, as in this case, the plots are presented in a panel for each catch or fishing mortality scenario reporting biomass, surplus production, catch and fishing mortality projections in each of the environmental scenarios. Note that, in this case, `plot_out=F` (by default), then plots are not saved like in the previous examples.
...
In this case, considering that the environmental information is not provided unlike the previous example, four plots are presented in a panel reporting the biomass, surplus production, catch and fishing mortality projections for each catch or fishing mortality scenario. 
