...
In this section the use of the `knobi_fit`, the function that allows us to run the KBPM fit, is illustrated using the northern hake (*Merluccius merluccius* ) as a case of study.
...
The estimated surplus production curves from the retrospective analysis are plotted. The plot is displayed in the plot window and saved if `plot_out=TRUE`.
...
In this case the first plot reports the correlation analysis between the environmental variable(s) and the base KBPM surplus production residuals. The last one reports the fitted values of the base model (no environmental information) and of the environmental ones.
