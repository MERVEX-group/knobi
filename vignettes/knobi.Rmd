...
There are many formulations of the production of biomass function *f(B<sub>t</sub>)*, among which the general Pella-Tomlinson (1969) is widely used:
...
* `RP`: Estimates of the biological reference points. 
  ...
  + `MSY`: KBPM estimate of maximum sustainable yield (MSY).
  + `B_MSY`: KBPM estimate of biomass at MSY.
  + `F_MSY`: KBPM estimate of fishing mortality at MSY.
...
* (3) the KBPM environmental fit, where environmental effects are included as additive and multiplicative effects in the KBPM formulation (equation 3).
...
The first plot reports the correlation analysis between the environmental variable(s) and the base KBPM surplus production residuals. The last one reports the fitted values of the base model (no environmental information) and of the environmental ones.
...
* `selected_lag`: Data frame with the estimated lag corresponding to the one reporting the highest correlation between the environmental variable and the base KBPM surplus production residuals and the correlation corresponding to this lag for each variable.
* `lag_cor`: Correlation between the environmental variable(s) value and the base KBPM surplus production residuals for each lag.
* `selected_var`: Environmental variable used in the fit derived from the highest pearson correlation procedure.
...
At last, there is also the possibility of testing the correlation between the KBPM SP residuals and the environmental variable(s) through the study of autoregressive models (AR models). In this case, an AR model is fitted for the residuals in order to determine, in the first place, how the residuals can explain themselves as follows

$$ r_t=\sum_{i=1}^{p}\beta_{i}r_{t-i}+\epsilon_{t}$$

being $r_t$ the KBPM base SP residual for year $t$ and $p$ the AR model order, estimated as the maximum partial autocorrelation function value that is bigger than $qnorm(0.975)/\sqrt(length(r))$ in absolute value.

Then, an AR model is fitted considering the lagged environmental variable(s) considering each variable and lag as follows

$$r_t=\sum_{i=1}^{p}\beta_{i}r_{t-i}+X_{t}+\epsilon_{t}$$

being $X_{t,lag}$ the lagged environmental variable, for each lag corresponding to $lag=\{0,1,...,nlag\}$.
...
* `end_y`: The final year of the projections. If not provided, it has to be provided `n_y` (the argument above). If both are provided, the function uses `n_y`.
